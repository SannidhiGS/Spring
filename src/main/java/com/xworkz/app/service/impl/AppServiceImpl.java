...
public class AppServiceImpl implements AppService {
    ...
    public boolean login(String email, String password) throws Exception {
        ...
        if (entity.isAccountLocked()) {
            throw new Exception("Account locked. Reset password using OTP.");
        }
    ...
    public void generateAndSendOtp(String email) {
        AppEntity appEntity=appDAO.findByEmail(email);
        if(appEntity==null){
            throw new RuntimeException("Data not found");
        }
        String otp=generateOtp();
        appEntity.setOtp(otp);
        appEntity.setOtpExpiry(LocalDateTime.now().plusMinutes(5));
        appEntity.setAccountLocked(true);
        appDAO.update(appEntity);
        mailService.sendOtpMail(appEntity.getEmail(),otp);
    }
